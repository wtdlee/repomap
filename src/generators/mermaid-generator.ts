import type { AnalysisResult, MermaidDiagram, PageInfo, DataFlow, ComponentInfo, CrossRepoLink } from "../types.js";
...
export class MermaidGenerator {
  ...
  generateDataFlowDiagram(result: AnalysisResult): MermaidDiagram {
    ...
    const getNodeShape = (type: string, name: string): [string, string] => {
    ...
    const contextFlows = result.dataFlows.filter(
      (f) => f.name.includes("🔄") || f.source.type === "context" || f.operations.some((o) => o.includes("Context"))
    );
