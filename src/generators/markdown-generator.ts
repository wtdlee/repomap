...
export class MarkdownGenerator {
  ...
  private generatePagesDoc(repo: RepositoryReport): string {
    ...
    for (const [category, pages] of byCategory) {
      ...
      for (const page of pages) {
        ...
        const remaining =
          queries.length + mutations.length - Math.min(queries.length, 2) - Math.min(mutations.length, 2);
        if (remaining > 0) {
  ...
  private formatGraphQLFields(fields: any[], indent: number): string {
  ...
  private getPageOperations(
    ...
    allOperations: GraphQLOperation[]
  ...
  private generateCrossRepoDoc(report: DocumentationReport): string {
    const lines: string[] = ["# クロスリポジトリ分析", "", "## アーキテクチャ概要", "", "```mermaid", "flowchart TB"];
  ...
  private createPageDataFlowDiagram(page: PageInfo, relatedComponents: ComponentInfo[], dataFlows: DataFlow[]): string {
    const lines: string[] = [];
    const maxWidth = 60;
    ...
    // Build ASCII diagram
    const pageName = page.path.split("/").pop() || "Page";
