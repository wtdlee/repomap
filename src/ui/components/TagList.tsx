...
import { QueryTag, MutationTag, FragmentTag } from './Tag.js';
...
export function TagList({ children, gap = 'sm', wrap = true, className }: TagListProps) {
  return (
  ...
  return (
    <TagList>
      {visible.map((op) => {
        ...
        if (op.type === 'query') {
          return (
            <QueryTag key={`q-${op.name}`} clickable={!!onClickOp} onClick={handleClick}>
              {op.name}
            </QueryTag>
          );
        }
        if (op.type === 'mutation') {
          return (
            <MutationTag key={`m-${op.name}`} clickable={!!onClickOp} onClick={handleClick}>
              {op.name}
            </MutationTag>
          );
        }
        return (
          <FragmentTag key={`f-${op.name}`} clickable={!!onClickOp} onClick={handleClick}>
            {op.name}
          </FragmentTag>
        );
      })}
      {remaining > 0 && <MoreButton onClick={onClickMore}>+{remaining} more</MoreButton>}
    </TagList>
  );
}
