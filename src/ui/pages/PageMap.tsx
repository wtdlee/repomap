...
import { Tag, AuthTag, PublicTag, RepoTag } from "../components/Tag.js";
...
const PageTypeTag = styled("span")<PageTypeTagProps>`
  ...
  background: ${(p) => {
    switch (p.$type) {
      case "PAGE": return "#e0e7ff";
      case "DYNAMIC": return "#fef3c7";
      case "LAYOUT": return "#dbeafe";
      default: return theme.colors.backgroundAlt;
    }
  }};
  color: ${(p) => {
    switch (p.$type) {
      case "PAGE": return "#3730a3";
      case "DYNAMIC": return "#92400e";
      case "LAYOUT": return "#1e40af";
...
function getPageType(path: string): string {
  ...
  if (path.includes("_app") || path.includes("_document") || path.includes("layout")) return "LAYOUT";
  return "PAGE";
}

// Helper to group pages
function groupPages(pages: Array<PageInfo & { repo?: string }>): Record<string, Array<PageInfo & { repo?: string }>> {
            ...
            return (
              <PageGroup key={group}>
                ...
                {pages.map((page) => {
                  ...
                  const queries = page.dataFetching?.filter((d) => d.type === "useQuery").length || 0;
                  const mutations = page.dataFetching?.filter((d) => d.type === "useMutation").length || 0;
                  const repoName = page.repo || "";
                  const shortRepo = repoName.split("/").pop()?.split("-").map((s: string) => s.substring(0, 4)).join("-") || repoName;
